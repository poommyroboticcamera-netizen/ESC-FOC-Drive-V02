...
#include "timer.h"
#include "bldc_control_noise.h"

#ifdef HW_BLDC_FRACTIONAL_ZERO
typedef float bldc_offset_t;
#else
typedef int bldc_offset_t;
#endif

// Sensorless BLDC normally uses the physical VSUPPLY ADC for flux-integrator
// scaling. Hardware without that ADC can provide a conservative equivalent.
#ifndef GET_BLDC_VBUS_ADC
#define GET_BLDC_VBUS_ADC() ((float)ADC_Value[ADC_IND_VIN_SENS])
#endif
...
static volatile bldc_offset_t curr0_offset;
static volatile bldc_offset_t curr1_offset;
static volatile mc_state state;
static volatile mc_control_mode control_mode;
static volatile float last_current_sample;
static volatile float last_current_sample_raw;
...
static volatile bldc_offset_t curr2_offset;
#endif
...
#ifdef HW_ADC_PRESCALER
	ADC_CommonInitStructure.ADC_Prescaler = HW_ADC_PRESCALER;
#else
...
void mcpwm_get_current_offsets(float *u, float *v, float *w) {
	// Offsets are immutable once boot calibration completes.
	*u = curr0_offset;
	*v = curr1_offset;
#ifdef HW_HAS_3_SHUNTS
	*w = curr2_offset;
...
#ifdef HW_BLDC_FRACTIONAL_ZERO
	// ADC interrupt owns accumulation; reset all sums as one transaction.
	const uint32_t cal_primask = __get_PRIMASK();
	__disable_irq();
#endif
	curr0_sum = 0;
	curr1_sum = 0;
...
#ifdef HW_BLDC_FRACTIONAL_ZERO
	__set_PRIMASK(cal_primask);
	// Exactly 4000 samples are latched by the ISR, then left unchanged.
	int cal_wait_ms = 0;
	while (curr_start_samples < 4000) {
		chThdSleepMilliseconds(1);
		if (++cal_wait_ms >= 2000) {
			DCCAL_OFF();
			// Leave dccal_done false: the software interlock must reject arm.
			return;
		}
	}
	curr0_offset = (float)curr0_sum / 4000.0f;
	curr1_offset = (float)curr1_sum / 4000.0f;
#ifdef HW_HAS_3_SHUNTS
	curr2_offset = (float)curr2_sum / 4000.0f;
#endif
#else
	while(curr_start_samples < 4000) {};
...
/**
 * Get the un-clamped BLDC current sample. This is intended for hardware fault
 * protection and diagnostics; control and ordinary telemetry keep using the
 * bounded sample returned by mcpwm_get_tot_current().
 */
float mcpwm_get_tot_current_raw(void) {
	return last_current_sample_raw;
}
...
static THD_FUNCTION(rpm_thread, arg) {
	...
	for (;;) {
		...
		rpm_dep.cycle_int_limit_running = rpm_dep.cycle_int_limit + GET_BLDC_VBUS_ADC() *
		...
		rpm_dep.cycle_int_limit_max = rpm_dep.cycle_int_limit + GET_BLDC_VBUS_ADC() *
...
static THD_FUNCTION(timer_thread, arg) {
	...
	for(;;) {
		...
		if (state == MC_STATE_OFF) {
			...
			if (direction == 1) {
				dutycycle_now = amp / GET_BLDC_VBUS_ADC();
			} else {
				dutycycle_now = -amp / GET_BLDC_VBUS_ADC();
			}
...
void mcpwm_adc_inj_int_handler(void) {
...
#ifdef HW_BLDC_FRACTIONAL_ZERO
	// Do not accumulate indefinitely (signed overflow), or adapt zero under load.
	if (!dccal_done && curr_start_samples < 4000) {
#endif
	curr0_sum += curr0;
	curr1_sum += curr1;
#ifdef HW_HAS_3_SHUNTS
	...
	if (conf->motor_type == MOTOR_TYPE_DC) {
		...
		#ifdef HW_BLDC_UW_SHUNTS_ONLY
			curr_tot_sample = bldc_uw_reconstructed_peak(curr0, curr2);
		#else
...
#ifdef HW_BLDC_UW_SHUNTS_ONLY
			curr_tot_sample = bldc_uw_current_sample(direction != 0,
					comm_step, curr0, curr2);
#elif defined(HW_HAS_3_SHUNTS)
			if (direction) {
				switch (comm_step) {
	...
	last_current_sample_raw = curr_tot_sample;
	last_current_sample = last_current_sample_raw;
...
void mcpwm_adc_int_handler(void *p, uint32_t flags) {
	...
	if (conf->motor_type == MOTOR_TYPE_BLDC) {
		...
		if (has_commutated) {
			amp = fabsf(dutycycle_now) * GET_BLDC_VBUS_ADC();
		} else {
		...
		if (sensorless_now) {
			...
			if ((state == MC_STATE_RUNNING && pwm_cycles >= 2) || state == MC_STATE_OFF) {
				...
				if (v_diff > 0) {
					// TODO!
					//					const int min = 100;
					int min = (int)((1.0 - fabsf(dutycycle_now)) * GET_BLDC_VBUS_ADC() * 0.3);
					if (min > GET_BLDC_VBUS_ADC() / 4.0) {
						min = (int)(GET_BLDC_VBUS_ADC() / 4.0);
					}

					if (pwm_cycles_sum > (last_pwm_cycles_sum / 2.0) ||
							!has_commutated || (ph_now_raw > min && ph_now_raw < (GET_BLDC_VBUS_ADC() - min))) {
		...
		if (has_commutated) {
			amp = dutycycle_now * GET_BLDC_VBUS_ADC();
		} else {
	...
	float current_limit_value = current_nofilter;
	float current_in_limit_value = current_in_nofilter;
#ifdef HW_BLDC_MAGNITUDE_CURRENT_LIMITS
	current_limit_value = bldc_current_limit_value(current_nofilter);
	current_in_limit_value = bldc_current_limit_value(current_in_nofilter);
#endif
	float current_control = current_nofilter;
#ifdef HW_BLDC_CONTROL_FILTER_TAU_S
	// Private to this ADC ISR: no cross-thread filter state or allocations.
	static float control_filtered = 0.0f;
	static int control_direction = -1;
	static bool control_valid = false;
	if (state != MC_STATE_RUNNING || !has_commutated) {
		control_valid = false;
	} else {
		if (!control_valid || control_direction != direction) {
			control_filtered = current_nofilter;
		} else {
			control_filtered = bldc_control_noise_update(control_filtered,
					current_nofilter, switching_frequency_now,
					HW_BLDC_CONTROL_FILTER_TAU_S);
		}
		control_direction = direction;
		control_valid = true;
		current_control = control_filtered;
	}
#endif
	...
	if (state == MC_STATE_RUNNING && has_commutated) {
...
#endif
			// Compute error
			const float error = bldc_current_control_error(current_set, current_control);
		...
		if (current_limit_value > conf->lo_current_max) {
			utils_step_towards((float*) &dutycycle_now, 0.0,
					ramp_step_no_lim * fabsf(current_limit_value - conf->lo_current_max) * conf->m_current_backoff_gain);
			limit_delay = 1;
		} else if (current_limit_value < conf->lo_current_min) {
			utils_step_towards((float*) &dutycycle_now, direction ? conf->l_max_duty : -conf->l_max_duty,
					ramp_step_no_lim * fabsf(current_limit_value - conf->lo_current_min) * conf->m_current_backoff_gain);
			limit_delay = 1;
		} else if (current_in_limit_value > conf->lo_in_current_max) {
			utils_step_towards((float*) &dutycycle_now, 0.0,
					ramp_step_no_lim * fabsf(current_in_limit_value - conf->lo_in_current_max) * conf->m_current_backoff_gain);
			limit_delay = 1;
		} else if (current_in_limit_value < conf->lo_in_current_min) {
			utils_step_towards((float*) &dutycycle_now, direction ? conf->l_max_duty : -conf->l_max_duty,
					ramp_step_no_lim * fabsf(current_in_limit_value - conf->lo_in_current_min) * conf->m_current_backoff_gain);
			limit_delay = 1;
		}
