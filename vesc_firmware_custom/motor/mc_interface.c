...
#ifdef HW_FORCE_MOTOR_TYPE
	// Apply a hardware-validated motor type before initializing the selected
	// motor-control implementation, even when flash contains an older setting.
	m_motor_1.m_conf.motor_type = HW_FORCE_MOTOR_TYPE;
#ifdef HW_FOC_BENCH_SAMPLING
	m_motor_1.m_conf.foc_f_zv = MCCONF_FOC_F_ZV;
	m_motor_1.m_conf.foc_control_sample_mode = FOC_CONTROL_SAMPLE_MODE_V0;
	m_motor_1.m_conf.foc_current_sample_mode = FOC_CURRENT_SAMPLE_MODE_LONGEST_ZERO;
#endif
#ifdef HW_HAS_DUAL_MOTORS
	m_motor_2.m_conf.motor_type = HW_FORCE_MOTOR_TYPE;
#endif
#endif
...
#ifdef HW_FORCE_MOTOR_TYPE
	configuration->motor_type = HW_FORCE_MOTOR_TYPE;
#ifdef HW_FOC_BENCH_SAMPLING
	configuration->foc_f_zv = MCCONF_FOC_F_ZV;
	configuration->foc_control_sample_mode = FOC_CONTROL_SAMPLE_MODE_V0;
	configuration->foc_current_sample_mode = FOC_CURRENT_SAMPLE_MODE_LONGEST_ZERO;
#endif
#endif
...
__attribute__((aligned(16))) void mc_interface_mc_timer_isr(bool is_second_motor, float dt) {
	...
	float abs_current;
#if !defined(HW_ABS_OVERCURRENT_HARD_LIMIT) || !defined(HW_ABS_OVERCURRENT_DEBOUNCE_SAMPLES)
...
#ifdef HW_BLDC_RAW_CURRENT_PROTECTION
		abs_current = mcpwm_get_tot_current_raw();
#else
	...
	// Current fault code. A custom low-current board can require a few
	// consecutive over-limit samples to reject one-sample switching spikes,
	// while retaining an independent immediate emergency cutoff.
#if defined(HW_ABS_OVERCURRENT_HARD_LIMIT) && defined(HW_ABS_OVERCURRENT_DEBOUNCE_SAMPLES)
	static bldc_oc_state_t abs_overcurrent_state[2] = {{0, 0}, {0, 0}};
	const unsigned int motor_index = is_second_motor ? 1U : 0U;
	const float abs_current_now = fabsf(abs_current);
	float emergency_current_now = abs_current_now;
#ifdef HW_FOC_RAW_CURRENT_PEAK
	if (conf_now->motor_type == MOTOR_TYPE_FOC) {
		emergency_current_now = fmaxf(emergency_current_now,
				HW_FOC_RAW_CURRENT_PEAK(conf_now->foc_offsets_current));
	}
#endif

	// BLDC current offsets are accumulated after the timer ISR starts. Before
	// DC calibration is complete the calculated current is not meaningful, and
	// PWM is not running. Do not log a false emergency fault during that window.
	if (state != MC_STATE_RUNNING || !mc_interface_dccal_done()) {
		abs_overcurrent_state[motor_index].normal_count = 0;
		abs_overcurrent_state[motor_index].emergency_count = 0;
	} else {
		const bldc_oc_result_t oc_result = bldc_overcurrent_update_separate(
				&abs_overcurrent_state[motor_index], abs_current_now, emergency_current_now,
				conf_now->l_abs_current_max,
				HW_ABS_OVERCURRENT_DEBOUNCE_SAMPLES,
				HW_ABS_OVERCURRENT_HARD_LIMIT,
				HW_ABS_OVERCURRENT_HARD_DEBOUNCE_SAMPLES);
		if (oc_result == BLDC_OC_EMERGENCY) {
		mc_interface_set_fault_info(
				"Current raw peak %.2f A exceeded emergency %.2f A",
				2, emergency_current_now, HW_ABS_OVERCURRENT_HARD_LIMIT);
		mc_interface_fault_stop(FAULT_CODE_ABS_OVER_CURRENT, is_second_motor, true);
		} else if (oc_result == BLDC_OC_SUSTAINED) {
			mc_interface_set_fault_info(
					"Current magnitude %.2f A persisted; filtered %.2f A",
					2, abs_current_now, current_filtered);
			mc_interface_fault_stop(FAULT_CODE_ABS_OVER_CURRENT, is_second_motor, true);
		}
	}
#else
#ifdef HW_ABS_OVERCURRENT_HARD_LIMIT
	if (fabsf(abs_current) > HW_ABS_OVERCURRENT_HARD_LIMIT) {
		mc_interface_fault_stop(FAULT_CODE_ABS_OVER_CURRENT, is_second_motor, true);
	} else
#endif
...
#ifdef HW_BLDC_RAW_CURRENT_PROTECTION
			if (motor->m_conf.motor_type == MOTOR_TYPE_BLDC) {
				fdata.current = mcpwm_get_tot_current_raw();
			}
#endif
