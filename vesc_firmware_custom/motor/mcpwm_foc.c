...
#ifdef HW_FOC_I_ABS_FILTER_MAGNITUDE
	// Filtering the already-positive magnitude avoids cancellation when the
	// measured d/q current alternates sign around zero.
	UTILS_LP_FAST(state_m->i_abs_filter, state_m->i_abs,
			conf_now->foc_current_filter_const);
#else
