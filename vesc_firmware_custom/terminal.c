...
#ifdef HW_DISABLE_FOC_OPENLOOP_ALL
		commands_printf("Disabled on this hardware: firmware is locked to sensorless BLDC six-step.");
		commands_printf("Use bldc_current [amps] after bldc_arm CONFIRM.");
#else
		if (argc == 3) {
			float current = -1.0;
			float erpm = -1.0;
...
#if defined(HW_DISABLE_FOC_OPENLOOP_DUTY) || defined(HW_DISABLE_FOC_OPENLOOP_ALL)
		commands_printf("Disabled on this hardware: open-loop voltage can exceed the bring-up current limit.");
		commands_printf("Use bldc_current [amps] after bldc_arm CONFIRM.");
#else
		if (argc == 3) {
			float duty = -1.0;
...
#ifdef HW_DISABLE_FOC_OPENLOOP_ALL
		commands_printf("  Disabled: this firmware is locked to sensorless BLDC six-step.");
#else
		commands_printf("  Create an open loop rotating current vector.");
#endif

#if defined(HW_DISABLE_FOC_OPENLOOP_DUTY) || defined(HW_DISABLE_FOC_OPENLOOP_ALL)
		commands_printf("foc_openloop_duty [disabled on this hardware]");
		commands_printf("  Use bldc_current [amps] after bldc_arm CONFIRM.");
#else
